...
- `docs/00-governance/ENGINEERING_WORK_ORDER_STANDARD.md` defines the normative engineering work order standard for all future implementation packages; adopted and released as governance baseline `ENGINEERING-WORKFLOW-STANDARD-1.0`.
...
- `docs/03-implementation/work-orders/` contains the architect-level implementation planning packages governed by the engineering work order standard (released as `ENGINEERING-WORKFLOW-STANDARD-1.0`); the current planning package is `ENGINEERING-ASSERTION-PROMOTION-0.1-RC` (planned, not begun).
...
- `ENGINEERING-WORKFLOW-STANDARD-1.0` is the latest released governance baseline (Engineering Work Order Standard adopted and normative).

## Next Active Package

- `ENGINEERING-ASSERTION-PROMOTION-0.1-RC` — planned; implementation has not begun; WO0 is the next action only after explicit instruction
